...
for comm in input_file.readlines():
	...
	if command[0] == "SDR":
		...
		if (device == 'ptx'):
			...
			temp_mask = command[5].split(')')[0].split('(') #command[5].rstrip(')').split('(')
			mask = int(temp_mask[1][0:2],16)
		...
		if (device == 'ptx'):
			...
			dev.write(inEndPoint,data_in_reverse,timeout)
		...
		if (( mask_int != 0 ) and ( mask != 0 )):	
			...
			if (device == 'ptx'):
				...
				for mask_bit in mask:
					if ( mask_bit == '1' ):
						if (inn[bit_num] != expct_out[bit_num]):
							invalid = 1
					bit_num += 1
			else:
			...
			file_data = expct_out + "    " + inn + "   " + result+"\n"
